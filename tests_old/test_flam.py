...
def test_acceleration_option_uses_single_key_without_boolean_aliases():
    ...
    flam_alias = chunkermat(
        chnkr,
        smooth_kernel,
        {"_chunkie_normalized_operator_options": True, "flam": True, "dval": 4.0},
    )
    fmm_alias = chunkermat(
        chnkr, smooth_kernel, {"_chunkie_normalized_operator_options": True, "fmm": True}
    )
    ...
    with pytest.raises(ValueError, match="acceleration must be one of"):
        chunkermat(
            chnkr,
            smooth_kernel,
            {"_chunkie_normalized_operator_options": True, "acceleration": "fast"},
        )
...
def test_flam_kernbyindex_matches_dense_and_sparse_overwrites():
    ...
    overwrite = sparse.csr_matrix(
        (np.array([9.0, -4.0]), (np.array([3, 7]), np.array([2, 7]))), shape=dense.shape
    )
...
def test_flam_accepts_explicit_chunker_sequences():
    ...
    np.testing.assert_allclose(
        entries, chunkermat(merged, smooth_kernel)[np.ix_(rows, cols)], atol=1e-14
    )
...
def test_flam_kernbyindexr_matches_dense_and_sparse_overwrites():
    ...
    overwrite = sparse.csr_matrix(
        (np.array([7.0, -3.0]), (np.array([1, 2]), np.array([3, 5]))), shape=dense.shape
    )
...
def test_flam_proxy_square_geometry_and_proxyfun_shapes():
    ...
    Kpxy, nbr_out = flam.proxyfun(
        slf, nbr, np.array([1.0, 1.0]), np.zeros(2), chnkr, smooth_kernel, (1, 1), pr, ptau, pw, pin
    )
    ...
    expected_bottom = (
        smooth_kernel(proxyinfo, PointInfo(r=srcinfo.r[:, slf])) * pw.reshape(1, -1)
    ).T
...
def test_chunkermat_flam_applies_solves_and_logdet_against_dense(test_metrics):
    chnkr, _ = chunkerfunc(circle, min_chunks=4, order=6)
    lap_s = kernel("lap", "s")
    ...
    dense, dense_elapsed = timed_call(lambda: chunkermat(chnkr, lap_s) + np.eye(chnkr.npt))
    flam_mat, flam_setup_elapsed = timed_call(lambda: chunkermat(chnkr, lap_s, opts))
    flam_applied, flam_apply_elapsed = timed_call(lambda: flam_mat @ rhs)
    dense_applied, dense_apply_elapsed = timed_call(lambda: dense @ rhs)

    assert isinstance(flam_mat, ChunkerFLAMMatrix)
    np.testing.assert_allclose(flam_applied, dense_applied, rtol=1e-10, atol=1e-11)
    sol, solve_elapsed = timed_call(lambda: flam_mat.solve(rhs))
    np.testing.assert_allclose(dense @ sol, rhs, rtol=1e-10, atol=1e-11)
    sign, logabs = np.linalg.slogdet(dense)
    np.testing.assert_allclose(
        flam_mat.logdet(), np.log(np.asarray(sign, dtype=complex)) + logabs, rtol=1e-10, atol=1e-10
    )
    test_metrics.record("chunkermat_flam_setup_elapsed_s", flam_setup_elapsed)
    test_metrics.record("chunkermat_flam_dense_build_elapsed_s", dense_elapsed)
    test_metrics.record("chunkermat_flam_solve_elapsed_s", solve_elapsed)
    record_backend_metrics(
        test_metrics,
        "chunkermat_flam_apply",
        backend="flam",
        problem_size={"source_nodes": chnkr.npt, "target_nodes": chnkr.npt, "rhs_columns": 1},
        elapsed_s=flam_apply_elapsed,
...
def test_chunkermat_flam_adjoint_products_match_dense():
    ...
    np.testing.assert_allclose(
        rskelf_mat.H @ rhs_mat, dense.conj().T @ rhs_mat, rtol=1e-10, atol=1e-11
    )
...
def test_chunkermat_flam_adds_dval_without_replacing_smooth_diagonal():
    ...
    flam_scalar = chunkermat(
        chnkr, smooth_kernel, acceleration="flam", dval=0.5, rank_or_tol=1e-10, proxy=False
    )

    np.testing.assert_allclose(
        flam_scalar @ rhs_scalar, dense_scalar @ rhs_scalar, rtol=1e-10, atol=1e-11
    )

    dense_complex = chunkermat(chnkr, smooth_kernel) + (0.5 + 0.2j) * np.eye(chnkr.npt)
    flam_complex = chunkermat(
        chnkr, smooth_kernel, acceleration="flam", dval=0.5 + 0.2j, rank_or_tol=1e-10, proxy=False
    )
    np.testing.assert_allclose(
        flam_complex @ rhs_scalar, dense_complex @ rhs_scalar, rtol=1e-10, atol=1e-11
    )

    dense_vector = chunkermat(chnkr, vector_smooth_kernel) + 0.25 * np.eye(2 * chnkr.npt)
    rhs_vector = np.sin(np.arange(2 * chnkr.npt))
    flam_vector = chunkermat(
        chnkr, vector_smooth_kernel, acceleration="flam", dval=0.25, rank_or_tol=1e-10, proxy=False
    )

    np.testing.assert_allclose(
        flam_vector @ rhs_vector, dense_vector @ rhs_vector, rtol=1e-10, atol=1e-11
    )
...
def test_chunkermat_flam_l2scale_matches_scaled_dense_matrix():
    ...
    weights = chnkr.wts.reshape(-1, order="F")
    scaled_dense = np.sqrt(weights)[:, None] * weighted_dense * (1.0 / np.sqrt(weights))[
        None, :
    ] + 0.75 * np.eye(chnkr.npt)
    rhs = np.sin(np.arange(chnkr.npt))
    flam_mat = chunkermat(
        chnkr,
        smooth_kernel,
    ...
    scaled_special = np.sqrt(weights)[:, None] * special_dense * (1.0 / np.sqrt(weights))[
        None, :
    ] + np.eye(chnkr.npt)
    flam_special = chunkermat(
        chnkr, lap_s, acceleration="flam", dval=1.0, l2scale=True, rank_or_tol=1e-10, proxy=False
    )
...
def test_chunkerkerneval_flam_preserves_target_data_without_proxy():
    ...
    flam_vals = chunkerkerneval(chnkr, target_data_kernel, dens, targets, opts).reshape(
        -1, order="F"
    )
...
def test_chunkerkerneval_flam_matches_eval_matrix_and_dense():
    ...
    flam_vec_vals = chunkerkerneval(chnkr, vector_smooth_kernel, dens_vec, targets, opts).reshape(
        -1, order="F"
    )
...
def test_chunkerkerneval_flam_complex_target_eval_matches_dense():
    ...
    flam_vals = chunkerkerneval(chnkr, complex_smooth_kernel, dens, targets, opts).reshape(
        -1, order="F"
    )
