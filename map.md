...
- Verification snapshot: `uv run pytest -q` on 2026-05-16,
  `24 passed` in 1.04 seconds; `uv run ruff check .` passes; `uv run mypy`
...
| `chunkie.quadrature` | in progress | Dense panel helpers, component-major dense operator materialization, and adaptive source-panel fallback are active; Helsing-Ojala and GGQ parity are required upcoming milestones. |
