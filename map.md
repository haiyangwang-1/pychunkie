...
| `chunkerflam` | ⚠️ ✅ 🧪 | `chunkerflam.m` | Builds PyFLAM `rskelf`/`rskel` factors using 0-based matrix callbacks and optional proxy compression for both factor types; scalar, vector-opdim, proxy-enabled, and smooth diagonal-shift paths are Python-tested. |
