...
- Verification snapshot: `uv run pytest -q` on 2026-05-16,
  `10 passed` in 0.36 seconds; `uv run ruff check .` passes; `uv run mypy`
...
| `chunkie.system` | scaffolded | Density layout and high-level system records are active; dense assembly/solve follows geometry and kernels. |
