...
- Verification snapshot: `uv run pytest -q` on 2026-05-16,
  `21 passed` in 1.31 seconds; `uv run ruff check .` passes; `uv run mypy`
...
| `chunkie.kernels` | in progress | Kernel object, Laplace/Helmholtz/biharmonic/Stokes formulas, registry, algebra, and singularity metadata foundation are active. Laplace metadata uses `G`, `G_a`, and `G_ab`; Helmholtz metadata differentiates `J0(k*rho) * G`; biharmonic metadata uses `B=-(rho^2/4)G`; Stokes velocity metadata covers `s` and `d`; algebra scales and cancels exact scalar/matrix singular terms. |
