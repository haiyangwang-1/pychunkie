...
- Verification snapshot: `uv run pytest -q` on 2026-05-16,
  `17 passed` in 1.21 seconds; `uv run ruff check .` passes; `uv run mypy`
...
| `chunkie.kernels` | in progress | Kernel object, Laplace/Helmholtz formulas, registry, algebra, and singularity metadata foundation are active. Laplace metadata uses `G`, `G_a`, and `G_ab`; Helmholtz metadata differentiates `J0(k*rho) * G`; algebra scales and cancels exact scalar/matrix singular terms. |
