...
- Verification snapshot: `uv run pytest -q` on 2026-05-17,
  `66 passed` in 1.91 seconds; `uv run ruff check .` passes; `uv run mypy`
...
| Area | Status | Notes |
| --- | --- | --- |
| `chunkie.geometry` | in progress | Panel-major `Chunker`, point maps, basic constructors, metrics, affine/rotate/reflect transforms, near-panel node-distance and rectangle flags, and operational `ChunkGraph` records/views/classification are the first active layer. |
