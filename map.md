...
- Verification snapshot: `uv run pytest -q` on 2026-05-16,
  `26 passed` in 1.36 seconds; `uv run ruff check .` passes; `uv run mypy`
...
| `chunkie.quadrature` | in progress | Dense panel helpers, component-major dense operator materialization, adaptive source-panel fallback, and initial Helsing-Ojala log-product weights are active; GGQ parity and full singularity-dispatch coverage are required upcoming milestones. |
