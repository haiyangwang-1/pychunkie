...
- Verification snapshot: `uv run pytest -q` on 2026-05-16,
  `43 passed` in 1.92 seconds; `uv run ruff check .` passes; `uv run mypy`
...
| Area | Status | Notes |
| --- | --- | --- |
| `chunkie.geometry` | in progress | Panel-major `Chunker`, point maps, basic constructors, and operational `ChunkGraph` records/views/classification are the first active layer. |
...
| `chunkie.rcip` | in progress | Dyadic local corner geometry, prolongation matrices, and density interpolation are active. Compression recursion and system integration remain required upcoming work. |
| `chunkie.system` | in progress | Density layout, dense one-unknown/one-equation assembly, dense/FLAM solve, dense/FMM evaluation, adaptive/Helsing-Ojala/GGQ panel replacement corrections, and `LaplaceExteriorDirichletSystem` are active for the first Laplace circle regression. Trace blocks use the quadrature operator-matrix adapter. |
| `chunkie.system.backends` | in progress | FMM2D evaluates scalar Laplace single- and double-layer potentials against dense references. FLAM factors dense reference matrices with `pyflam.rskelf` and tests apply/solve against dense linear algebra. |
