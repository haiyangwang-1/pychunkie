...
- Verification snapshot: `uv run pytest -q` on 2026-05-16,
  `42 passed` in 2.30 seconds; `uv run ruff check .` passes; `uv run mypy`
...
| `chunkie.system` | in progress | Density layout, dense one-unknown/one-equation assembly, dense/FLAM solve, dense/FMM evaluation, panel replacement corrections, and `LaplaceExteriorDirichletSystem` are active for the first Laplace circle regression. Trace blocks use the quadrature operator-matrix adapter. |
