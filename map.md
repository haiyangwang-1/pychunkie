...
Verification snapshot: `uv run pytest` on 2026-05-12 with Python 3.11.9 collected 298 tests: `298 passed`. Full MATLAB parity runs generate ignored `tests/golden/*.mat` files on demand and require a populated `external/chunkie-matlab` checkout.
...
| `ChunkerFLAMMatrix` | ⚠️ ✅ 🧪 | `chunkerflam.m`, `+chnk/+flam/*` concepts | Matrix-free `LinearOperator` returned by `chunkermat(..., {"acceleration": "flam"})`; supports vector and multiple-RHS products, exposes `.factor`, `.solve(rhs)`, `.logdet()`, and dense materialization helpers when backed by PyFLAM `rskelf`. Full multi-chunker block-kernel parity remains pending. |
...
| `chunkerkerneval` | ✅ 🧪 🎯 | `chunkerkerneval.m` | MATLAB parity fixture checks dense target evaluation, including `forceadap` close-target replacement for Laplace Green-identity devtools targets; FLAM target evaluation is Python-tested, with `forceadap=True` using PyFLAM smooth evaluation plus sparse adaptive near-target corrections. |
| `chunkerkernevalmat` | ✅ 🧪 🎯 | `chunkerkernevalmat.m` | MATLAB parity fixture checks eval matrices, including adaptive close-target replacement through `forceadap`; FLAM eval-matrix materialization is Python-tested, with `forceadap=True` using PyFLAM smooth materialization plus sparse adaptive near-target corrections. |
