...
def test_low_level_pquad_weights_match_oversampled_legendre_moments():
    nsrc = 16
    ...
    for degree in (0, 1, 3, 7):
        ...
        for weights0, expected0 in zip(special, expected, strict=True):
            np.testing.assert_allclose(weights0 @ node_values, expected0, rtol=1e-12, atol=1e-12)


def test_pquad_panel_weights_can_compose_to_original_nodes():
    ...
    for upsampled_weights, original_weights in zip(upsampled, original, strict=True):
        np.testing.assert_allclose(original_weights, upsampled_weights @ interp, atol=1e-14)
...
def test_pquad_split_panel_matrix_matches_oversampled_legendre(kernel_args, side, normal_sign):
    chnkr, _ = chunkerfunc(circle, {"nchmin": 8}, {"k": 8})
    src_chunk = 0
    mid = lege.matrin(chnkr.k, [0.0])[0]
    rmid = (mid @ chnkr.r[:, :, src_chunk].T).T[:, 0]
