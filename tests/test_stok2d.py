...
def test_stokes_kernel_shapes():
    ...
    single = stok2d.kern(mu, src, targ, "s")
    spres = stok2d.kern(mu, src, targ, "spres")
    ...
    d_dx = (stok2d.kern(mu, src, txp, "s") - stok2d.kern(mu, src, txm, "s")) / (2.0 * eps)
    d_dy = (stok2d.kern(mu, src, typ, "s") - stok2d.kern(mu, src, tym, "s")) / (2.0 * eps)
    ...
    np.testing.assert_allclose(stok2d.kern(mu, src, targ, "sgrad"), expected_sgrad, rtol=1e-5, atol=1e-7)
