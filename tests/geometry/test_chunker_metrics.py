...
def test_circle_metrics_match_analytic_geometry():
    ...
    np.testing.assert_allclose(boundary.panel_lengths, np.full(6, 2.0 * np.pi * 2.5 / 6), atol=1.0e-13)
...
def test_translation_and_uniform_scaling_preserve_layout_conventions():
    ...
    np.testing.assert_allclose(moved.positions, boundary.positions + np.array([1.0, -2.0])[:, None, None])
...
def test_affine_rotation_and_reflection_update_normals_weights_and_area():
    ...
    expected_positions = np.einsum("ij,jsS->isS", matrix, boundary.positions) + np.array([0.5, -0.25])[
        :, None, None
    ]
    ...
    expected_normals = np.stack((expected_derivatives[1] / expected_speed, -expected_derivatives[0] / expected_speed))
    ...
    np.testing.assert_allclose(transformed.second_derivatives, np.einsum("ij,jsS->isS", matrix, boundary.second_derivatives))
    np.testing.assert_allclose(transformed.normals, expected_normals)
    np.testing.assert_allclose(transformed.weights, expected_speed * boundary.reference_weights[:, None])
    np.testing.assert_allclose(transformed.area, np.linalg.det(matrix) * boundary.area, atol=1.0e-12)
    ...
    reflected = reflect(boundary, reflection_angle, center=source_center, target_center=target_center)
    np.testing.assert_allclose(
        reflected.positions,
