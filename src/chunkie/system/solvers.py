...
def solve_system(system, *, config: SystemConfig) -> SystemSolution:
    ...
    elif config.solve_method == "dense":
...
def _gmres_solve(matrix, rhs, config: SystemConfig):
    ...
    # operators can reuse the same solver boundary once their correction and
    # RCIP paths are complete.
    vector, info = gmres(
        operator,
        rhs,
