...
def _helmholtz_splitinfo(kind: str, zk: Any, coefs: Any, opdims: tuple[int, int], scale: Any = 1.0) -> SplitInfo | None:
    ...
    if kind in {"d", "double"}:
        return SplitInfo((SMOOTH, LOG, CAUCHY), ("r", "r", "r"), lambda s, t: _scale_split(scale, _helmholtz_d_split(helm2d, zk, s, t)), (1, 1))
    if kind in {"c", "combined"}:
        c = np.array([1.0, 1.0j]) if coefs is None else np.asarray(coefs).reshape(-1, order="F")
        return SplitInfo((SMOOTH, LOG, CAUCHY), ("r", "r", "r"), lambda s, t: _scale_split(scale, _helmholtz_c_split(helm2d, zk, c, s, t)), (1, 1))
    return None
