...
| 58 | `kernel_interleaveTest.m` | ✅ 🧪 🚧 ⚠️ | Very Hard | Tests invalid interleave detection, then solves an exterior Neumann problem with a block interleaved Helmholtz representation and fast-direct interfaces. | Python now covers smooth interleaved block-kernel FLAM application against dense products. Port remaining stages: invalid interleave fixture, Helmholtz block matrix entries, dense solve, FMM paths where applicable, and full fast-direct solve parity. |
