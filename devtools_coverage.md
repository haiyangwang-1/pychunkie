...
| 24 | `slicegraphTest.m` | ✅ 🧪 🎯 ⚠️ | Medium | Builds concentric-square chunkgraphs, slices selected edges, checks sliced geometry, compares sliced system matrix to full submatrix, and verifies edge id ordering. | Covered in `devtools_easy.mat`: compare sliced geometry and `edgeids`, and verify MATLAB and Python each preserve the inner sliced/full-submatrix relation. Python graph matrices now have finite Laplace double-layer self blocks; direct dense matrix-value parity remains partial because MATLAB default graph `chunkermat` applies RCIP corrections not yet present in the Python scalar graph path. |
...
1. Resolve strict chunkgraph matrix-value parity for `chunkermatapplyTest.m`
